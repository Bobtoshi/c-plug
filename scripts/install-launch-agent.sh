...
PROJECT_DIR="${0:A:h:h}"
RUNTIME_DIR="$HOME/Library/Application Support/KStack"
NODE_BIN="$(command -v node)"
CODEX_BIN="$(command -v codex || true)"
RUNTIME_PATH="${NODE_BIN:h}:${CODEX_BIN:h}:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin:/usr/sbin:/sbin"
TARGET="$HOME/Library/LaunchAgents/com.kstack.agent.plist"
TEMPLATE="$PROJECT_DIR/scripts/com.kstack.agent.plist.template"
...
plutil -lint "$TARGET"
launchctl bootout "gui/$UID/com.kstack.agent" 2>/dev/null || true
sleep 1
if ! launchctl bootstrap "gui/$UID" "$TARGET"; then
  sleep 1
  launchctl bootstrap "gui/$UID" "$TARGET"
fi
launchctl enable "gui/$UID/com.kstack.agent"
echo "Installed com.kstack.agent from $RUNTIME_DIR. Open http://127.0.0.1:4317 to finish iMessage pairing."
